...
import com.server.concert_reservation.application.payment.PaymentUseCase;
...
public class PaymentController {
    ...
    public ResponseEntity<PaymentHttpResponse.PaymentResponse> payment(@RequestBody PaymentHttpRequest.PaymentRequest request, @RequestHeader("X-Waiting-Token") String token) {
        return ResponseEntity.ok(PaymentHttpResponse.PaymentResponse.of(paymentUseCase.payment(PaymentCommand.from(request, token))));
    }
}
